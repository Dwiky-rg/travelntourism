...
const ModalHotel = ({ isModalOpen, selectedHotel, closeModal }) => {
  ...
  const calculateHotelTotalPrice = () => {
    return selectedRoom ? selectedRoom.finalPrice * numNights : 0;
  };
    ...
    <>
      {!isPaymentModalOpen && !isSuccessModalOpen && !isInvoiceModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50" onClick={closeModal}>
          <div className="bg-white rounded-lg w-11/12 max-w-4xl p-6 flex flex-col md:flex-row relative shadow-lg" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeModal} className="absolute top-4 right-4 cursor-pointer text-2xl hover:text-red-500 transition duration-300">
              <FaTimes />
            </button>

            <div className="w-full md:w-1/2 flex-shrink-0">
              <img src={selectedHotel.imageUrl} alt={`${selectedHotel.name} hotel`} className="w-full h-[465px] object-cover rounded-lg shadow-md" />
            </div>
            ...
            <div className="w-full md:w-1/2 md:ml-6 mt-3">
              ...
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-gray-700">Select Room</h3>
                <select
                  id="roomType"
                ...
                >
                  {rooms.map((room) => (
                    <option key={room.type} value={room.type} className="text-gray-700 hover:bg-blue-100 hover:text-blue-600 transition duration-200">
                      {room.type}
                    </option>
                  ))}
                </select>
              ...
              {selectedRoom && (
                <div className="mt-4 text-gray-700">
                  <p>
                    Room type: <span className="font-medium">{selectedRoom.type}</span>
                  </p>
                  <p>
                    Price per night: <span className="font-semibold text-blue-600">IDR {selectedRoom.finalPrice.toLocaleString()}</span>
                  </p>
                </div>
              )}

              <div className="mt-4">
                <label htmlFor="numNights" className="block text-sm font-semibold text-gray-700">
              ...
              <button
                className="w-full text-white py-2 rounded-lg mt-6 bg-blue-500 hover:bg-blue-600 transition duration-300"
                onClick={() => setPaymentModalOpen(true)} // Buka modal pembayaran
              >
                Continue to Payment
              </button>
            </div>
      ...
      {isPaymentModalOpen && (
        <ModalPayment
          ...
          onPaymentSuccess={handlePaymentSuccess} // Setelah sukses, tampilkan modal sukses
        />
      )}
        ...
        >
          ...
          >
            <Invoice
              ...
              onClose={closeModal} // Fungsi untuk menutup modal invoice
            />
          </div>
        </div>
      )}
