...
const PesawatForm = () => {
  ...
  const fetchPesawats = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/pesawat`);
  ...
  const handleAddPesawat = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/pesawat/add`, form, {
        headers: {
  ...
  const handleUpdatePesawat = async (e) => {
    ...
    try {
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/pesawat/update/${currentPesawat.id}`, form, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
  ...
  const handleDeletePesawat = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(`${import.meta.env.VITE_API_URL}/pesawat/${id}`);

          if (response.status === 200) {
            setMessage({ text: "Pesawat deleted successfully", type: "success" });
            fetchPesawats();
          } else {
  ...
  return (
    <div className="container mx-auto p-6 text-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-semibold text-gray-100 mb-6">Pesawat Management</h1>

      {message.text && <div className={`mb-6 p-4 text-white rounded-lg ${message.type === "success" ? "bg-green-500" : "bg-red-500"}`}>{message.text}</div>}
      ...
      <button
        ...
        className="bg-blue-600 text-white py-3 px-6 rounded-lg shadow-md transition duration-200 ease-in-out hover:bg-blue-700 mb-6"
      >
        Add New Pesawat
      </button>
        ...
        <table className="min-w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            {pesawats.map((pesawat) => (
              <tr key={pesawat.id} className="hover:bg-gray-800 transition duration-200">
                <td className="p-2 text-gray-200">{pesawat.airline}</td>
                <td className="p-2">
                  <img src={`/images-airlines/${pesawat.logo}`} alt="logo" className="w-16 h-16 object-contain" />
                </td>
                <td className="px-4 py-10 flex items-center space-x-4 justify-center">
                  ...
                  <button onClick={() => handleDeletePesawat(pesawat.id)} className="text-red-500 hover:text-red-600">
                    <FaTrash size={20} />
                  </button>
                </td>
              </tr>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-lg">
            <h2 className="text-xl font-semibold text-white mb-4">{currentPesawat ? "Edit Pesawat" : "Add New Pesawat"}</h2>
            <form onSubmit={currentPesawat ? handleUpdatePesawat : handleAddPesawat} className="space-y-6">
              <div>
                <input
                  type="text"
                  name="airline"
              ...
              <div className="flex justify-end space-x-4">
                <button type="button" onClick={() => setShowModal(false)} className="bg-red-500 text-white py-2 px-4 rounded-lg hover:bg-red-600">
                  Cancel
                </button>
